...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Explicitly clear only the latched reBot 200204 JOINT_FEEDBACK_TIMEOUT fault. "
            "This script never enables motors and never sends a motion command."
        )
    )
    ...
    parser.add_argument(
        "--confirm",
        required=True,
        help="must be exactly RESET-200204",
    )
...
def main() -> int:
    args = parse_args()
    if args.confirm != "RESET-200204":
        raise SystemExit("--confirm must be exactly RESET-200204")

    Robot = _load_robot(args.sdk_root)
    print(
        "FAULT_RESET_ONLY: connect/read/reset_fault/close; "
    ...
    with Robot(
        ...
        print(
            f"post_reset mode={_enum_text(state.robot_mode)} "
            f"safety={_enum_text(state.safety_state)} "
            f"active_faults={post_faults}"
        )
        print(
        ...
        if _enum_text(state.robot_mode) != "disabled":
            raise RuntimeError(
                "reset cleared the fault but did not confirm robot_mode=disabled"
            )

    print("RESULT: 200204 cleared; robot remains disabled")
    return 0
